...
class REVAnalyzer:
    def __init__(self, metric, image, size, cut_step, sREV_max_size, datadir='data', outputdir='output'):
        assert issubclass(
            self.metric.__class__, BasicMetric), "Metric should be an object of a class derived from BasicMetric"
        self.metric = metric
        self.image = image
        self.size = size
                ...
                if (l <= self.sREV_max_size):
                    ...
                    for cut_name in cut_names:
                        metric_cut_name = self.metric.generate(
                            self.metric.statoildir, cut_name, l, self.outputdir_cut_values)
                    ...
                    self._metric_cut_names.append(metric_cut_name)
                metric_cut_name = self.metric.generate(
                    self.metric.statoildir, self.image, self.size, self.outputdir_cut_values)
        else:
            for l in self.cut_sizes:
                cuts_data = 'cuts_data'
                ...
                if (l <= self.sREV_max_size):
                    ...
                    for cut_name in cut_names:
                        metric_cut_name = self.metric.generate(
                            cuts_data, cut_name, l, self.outputdir_cut_values)
                        self._metric_cut_names.append(metric_cut_name)
                    ...
                    shutil.rmtree(cuts_data)
            metric_cut_name = self.metric.generate(
                self.datadir, self.image, self.size, self.outputdir_cut_values)
            self._metric_cut_names.append(metric_cut_name)

    def read(self, cut_size, cut_id=0):
        data = self.metric.read(
            self.outputdir_cut_values, self.image, cut_size, cut_id)
        return data
    ...
    def show_results(self, figdir='figs'):
        ...
        if self.metric.metric_type == 'v':
            ...
            ax.plot(xsrev, ysrev1, "r-", label='sREV, $\sigma_{1}$')
            ax.plot(xsrev, ysrev2, "r--", label='sREV, $\sigma_{2}$')
